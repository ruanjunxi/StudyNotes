...
- 主从服务器第一次同步时，才用的就是全量复制。此时主服务器有两个耗时的地方，分别是生成RDB文件和传输RDB文件。为了避免过多从服务器和主服务器进行全量复制，可以把一部分从服务器升级为【经理角色】，并制定部分从服务器为他们的从服务器，从而减轻主服务器压力。
- 第一次同步之后，主服务器会维护一个TCP长链接，主服务器后续收到的写操作都会通过这个链接发送给从服务器，从而保证主从服务器数据一致。
- 如果遇到网络断开，而后网络又恢复了，从服务器重新上线，那么这段时间主服务器收到的写操作无法同步给从服务器，从而导致数据不一致问题。引入**增量复制**。
...
- 可以设置缓冲区大小为**second * write_size_per_second** ：断开的平均时长*redis每秒收到的写操作大小。

#### 哨兵机制

在Redis的主从架构中，如果主节点（master）挂了，那么没有主节点来响应客户端的写操作请求，也没有主节点对从节点（slave）进行数据同步了，因此2.8版本后引入哨兵机制，让主节点挂了后，哨兵会从剩余的从节点中选出新的主节点，并把新的主节点信息广播给其他从节点。

因此，哨兵主要的工作就是三个：监控，选主，通知，主要解决三个问题：
...
- 哨兵会每隔1秒给主节点发送一个ping命令，当给主节点收到这个ping命令后，会发送一个响应命令给哨兵。如果哨兵没有在规定时间内收到主节点发来的响应，则会把主节点标记为**【主观下线】**。

- 由于可能因为网络阻塞或主节点系统压力大导致响应超时，因此为了减少误判，一般会有多个哨兵构成**哨兵集群**，通过多个哨兵一起判断，从而避免单个哨兵因为本身网络不好而造成误判。
- 当一个哨兵认定主节点为主观下线后，会向其他哨兵发送`is-master-down-by-addr`命令，其他哨兵会根据自己和主节点的网络连接情况，做出赞成投票或者拒绝投票的响应。

#### 故障转移的过程

- 选出新的主节点；
  - 根据从节点的**优先级、复制进度、ID 号**来选出新的主节点。
- 修改其余从节点从属为新的主节点；
- 将新主节点的IP地址和信息，通过「发布者/订阅者机制」通知给客户端；
- 当旧的主节点上线后，将其设置为新主节点的从节点